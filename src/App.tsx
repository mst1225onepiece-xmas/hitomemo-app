...
import { BasicMemoEditForm, BasicMemoSummary, ConversationFields, Field, MeetBeforeMemoEditForm, MeetBeforeMemoSummary, PrivacyNote } from './components';
...
const ageOptions = ['20代前半', '20代中盤', '20代後半', '20歳前後', 'アラサー', '30代前半', '30代中盤', '30代後半'];
const genderOptions = ['男性', '女性', 'その他', '未記入'];
const heightOptions = ['小柄', '150cm台', '160cm前半', '160cm後半', '170cm前半', '175cm前後', '180cm前後', '高め'];
const jobOptions = ['会社員', 'IT系', '事務', '営業', '看護師', '保育士', '公務員', '学生', '接客', 'クリエイター', '自営業', '未記入'];
const mbtiOptions = ['INTJ', 'INTP', 'ENTJ', 'ENTP', 'INFJ', 'INFP', 'ENFJ', 'ENFP', 'ISTJ', 'ISFJ', 'ESTJ', 'ESFJ', 'ISTP', 'ISFP', 'ESTP', 'ESFP', '不明'];
...
function OptionField({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  ...
  return <div className="option-field"><Field label={label} value={value} onChange={onChange} /><button className="option-toggle" type="button" onClick={() => setOptionsOpen((open) => !open)}>{optionsOpen ? '候補を隠す' : '候補を表示'}</button>{optionsOpen && <div className="option-grid" aria-label={`${label}の候補`}>{options.map((option) => <button className="option-button" type="button" key={option} onClick={() => onChange(option)}>{option}</button>)}</div>}</div>;
}
...
function AddView({ data, persist, onDone }: { data: HitomemoData; persist: (data: HitomemoData) => void; onDone: () => void }) {
  ...
  return <form className="stack" onSubmit={submit}>{error && <p className="error">{error}</p>}<section className="panel"><h2>基本</h2><Field label="呼び名" required value={person.nickname} onChange={(value) => updatePerson('nickname', value)} /><Field label="関係" value={person.relation} onChange={(value) => updatePerson('relation', value)} /><button className="primary" type="submit">登録する</button></section><CollapsiblePanel title="会話のための特徴" open={featuresOpen} onToggle={() => setFeaturesOpen((open) => !open)}><div className="two-column-fields"><OptionField label="年齢" value={person.ageNote} options={ageOptions} onChange={(value) => updatePerson('ageNote', value)} /><OptionField label="性別" value={person.genderNote} options={genderOptions} onChange={(value) => updatePerson('genderNote', value)} /><OptionField label="身長" value={person.heightNote} options={heightOptions} onChange={(value) => updatePerson('heightNote', value)} /><OptionField label="職業" value={person.jobNote} options={jobOptions} onChange={(value) => updatePerson('jobNote', value)} /><OptionField label="MBTI" value={person.mbtiNote} options={mbtiOptions} onChange={(value) => updatePerson('mbtiNote', value)} /></div><Field label="見た目・雰囲気" hint="外見、話し方、声、服装、第一印象など、思い出すための外側の手がかり。" multiline value={person.appearanceNote} onChange={(value) => updatePerson('appearanceNote', value)} /><Field label="性格" multiline value={person.personalityNote} onChange={(value) => updatePerson('personalityNote', value)} /><Field label="好きなこと" multiline value={person.likesNote} onChange={(value) => updatePerson('likesNote', value)} /><Field label="苦手・嫌いなもの" multiline value={person.dislikesNote} onChange={(value) => updatePerson('dislikesNote', value)} /><Field label="その人らしさメモ" hint="考え方、価値観、距離感、空気感、内面っぽい印象など、分類しづらいその人らしさ。" multiline value={person.uniquenessNote} onChange={(value) => updatePerson('uniquenessNote', value)} /></CollapsiblePanel><CollapsiblePanel title="会う前のメモ" open={talkMemoOpen} onToggle={() => setTalkMemoOpen((open) => !open)} help="実際に会う前に、覚えておきたいことや話したいことを書く場所です。"><Field label="覚えておきたいこと" multiline value={person.rememberNote} onChange={(value) => updatePerson('rememberNote', value)} /><Field label="次に話したいこと" multiline value={person.nextTalkNote} onChange={(value) => updatePerson('nextTalkNote', value)} /><Field label="自分が感じていること" multiline value={person.feelingNote} onChange={(value) => updatePerson('feelingNote', value)} /><Field label="自由メモ" multiline value={person.memo} onChange={(value) => updatePerson('memo', value)} /></CollapsiblePanel><CollapsiblePanel title="初回会話メモ" open={firstConversationOpen} onToggle={() => setFirstConversationOpen((open) => !open)} help="実際に会ったあと、話した内容や感じたことを書く場所です。"><ConversationFields value={conversation} setValue={setConversation} /></CollapsiblePanel><button className="primary details-submit" type="submit">登録する</button><CollapsiblePanel title="このアプリの使い方・注意" open={privacyOpen} onToggle={() => setPrivacyOpen((open) => !open)}><PrivacyNote /></CollapsiblePanel></form>;
}
...
function ConversationCard({ conversation, data, persist, personId }: { conversation: Conversation; data: HitomemoData; persist: (data: HitomemoData) => void; personId: string }) {
  ...
  return <article className="conversation-card"><div className="conversation-head"><strong>{conversation.metDate}</strong><div className="button-row"><button className="secondary" type="button" onClick={() => setEditing(!editing)}>{editing ? '閉じる' : '編集'}</button><button className="danger" type="button" onClick={remove}><Trash2 size={15} />削除</button></div></div>{editing ? <div className="stack compact"><ConversationFields value={draft} setValue={setDraft} /><button className="primary" type="button" onClick={save}>保存する</button></div> : <dl className="memo-list small"><dt>場所・場面</dt><dd>{conversation.placeNote || '（未入力）'}</dd><dt>話したこと</dt><dd>{conversation.talkedAbout || '（未入力）'}</dd><dt>感じたこと・メモ</dt><dd>{conversationMemoText(conversation)}</dd></dl>}</article>;
}
